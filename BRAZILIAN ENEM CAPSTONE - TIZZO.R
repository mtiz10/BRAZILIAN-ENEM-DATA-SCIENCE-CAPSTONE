...
# Locate CSV file in directory if it already exists
found_files <- list.files(pattern = "RESULTADOS_2024.csv", recursive = TRUE, full.names = TRUE)
...
if (length(found_files) == 0 && !file.exists(rds_path_test) && !file.exists(rds_path_full)) {
  ...
  message("Dataset not found. Downloading ENEM 2024 ZIP (~700MB) from INEP... \n")
  options(timeout = 3000) 
  download.file(zip_url, destfile = zip_file, mode = "wb")
  
  message("Download completed. Unzipping files...")
  unzip(zip_file)
...
cat("Table 02 Raw Dataset Transposed: \n\n")
